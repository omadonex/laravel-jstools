...
declare global {
  interface Window {
    ...
    table: (
      tableId: string,
      columns: Array<AnyObjInterface>,
      scroll?: boolean,
      options?: AnyObjInterface,
...
window.tableFilterData = function (data, tableId) {
  ...
  $(`.${tableId}.filter.filter-input[type="checkbox"], #${tableId}.filter.filter-input[type="checkbox"]`).each(
    function () {
      const $this = $(this);
  ...
  $(
    `.${tableId} .filter .filter-input[data-jst-component], #${tableId} .filter .filter-input[data-jst-component]`,
  ).each(function () {
    const $this = $(this);
  ...
  $.each(
    $(`.${tableId} .filter .filter-input, #${tableId} .filter .filter-input`).serializeArray(),
...
window.modalConfirmDeleteTableRow = (
  rowCaption: string,
  url: string,
  table?: any,
  tableId?: string,
  ...
  window
    .modal(!!tableId ? `${tableId}_ModalConfirmDelete` : 'PartialsModalConfirmDelete', ModalUsageEnum.confirm, {
      bodyText: rowCaption,
      submitData: {
        url: url,
        method: 'DELETE',
...
window.initButtonCreate = (table: any, tableId: string, callback: any = null) => {
  $(`#${tableId}__btnCreate`).on('click', () => {
    ...
    const modalCreate = window.modal(
      modalId,
      ModalUsageEnum.form,
      Object.assign(
        {
